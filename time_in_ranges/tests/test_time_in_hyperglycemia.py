...
from time_in_ranges import time_in_hyperglycemia


def test_time_in_hyperglycemia():
    """
    Unit test of time_in_hyperglycemia function.
    ...
    #Tests
    assert np.isnan(time_in_hyperglycemia(data,'diabetes')) == False
    assert time_in_hyperglycemia(data, 'diabetes') == 40
    ...
    # Tests
    assert np.isnan(time_in_hyperglycemia(data))
