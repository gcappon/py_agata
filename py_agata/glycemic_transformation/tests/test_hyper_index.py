...
from py_agata.glycemic_transformation import hyper_index


def test_hyper_index():
    """
    Unit test of hyper_index function.
    ...
    #Tests
    assert np.isnan(hyper_index(data)) == False
    assert np.round(hyper_index(data)*1000)/1000 == 1.007
    ...
    # Tests
    assert np.isnan(hyper_index(data))
