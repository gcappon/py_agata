...
from py_agata.glycemic_transformation import grade_hyper_score


def test_grade_hyper_score():
    """
    Unit test of grade_hyper_score function.
    ...
    #Tests
    assert np.isnan(grade_hyper_score(data)) == False
    assert np.round(grade_hyper_score(data)*1000)/1000 == 48.293
    ...
    # Tests
    assert np.isnan(grade_hyper_score(data))
