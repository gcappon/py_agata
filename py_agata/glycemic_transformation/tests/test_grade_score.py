...
from py_agata.glycemic_transformation import grade_score


def test_grade_score():
    """
    Unit test of grade_score function.
    ...
    #Tests
    assert np.isnan(grade_score(data)) == False
    assert np.round(grade_score(data)*1000)/1000 == 14.525
    ...
    # Tests
    assert np.isnan(grade_score(data))
