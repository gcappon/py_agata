...
from py_agata.glycemic_transformation import hypo_index


def test_hypo_index():
    """
    Unit test of hypo_index function.
    ...
    #Tests
    assert np.isnan(hypo_index(data)) == False
    assert np.round(hypo_index(data)*1000)/1000 == 5.667
    ...
    # Tests
    assert np.isnan(hypo_index(data))
