...
from py_agata.variability import range_glucose


def test_range_glucose():
    """
    Unit test of range_glucose function.
    ...
    #Tests
    assert np.isnan(range_glucose(data)) == False
    assert np.round(range_glucose(data)*100)/100 == 220
    ...
    # Tests
    assert np.isnan(range_glucose(data))
