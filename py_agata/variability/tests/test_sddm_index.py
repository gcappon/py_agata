...
import os
from py_agata.variability import sddm_index


def test_sddm_index():
    """
    Unit test of sddm_index function.
    ...
    #Tests
    assert np.isnan(sddm_index(data)) == False
    assert np.round(sddm_index(data) * 1000) / 1000 == 4.326
    ...
    # Tests
    assert np.isnan(sddm_index(data))