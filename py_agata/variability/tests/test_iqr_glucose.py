...
from py_agata.variability import iqr_glucose


def test_iqr_glucose():
    """
    Unit test of iqr_glucose function.
    ...
    #Tests
    assert np.isnan(iqr_glucose(data)) == False
    assert np.round(iqr_glucose(data)*100)/100 == 142.50
    ...
    # Tests
    assert np.isnan(iqr_glucose(data))
