...
import os
from py_agata.variability import mage_plus_index


def test_mage_plus_index():
    """
    Unit test of mage_plus_index function.
    ...
    #Tests
    assert np.isnan(mage_plus_index(data)) == False
    assert np.round(mage_plus_index(data) * 1000) / 1000 == 160.507
    ...
    # Tests
    assert np.isnan(mage_plus_index(data))
