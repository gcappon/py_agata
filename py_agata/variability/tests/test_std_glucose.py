...
from py_agata.variability import std_glucose


def test_std_glucose():
    """
    Unit test of std_glucose function.
    ...
    #Tests
    assert np.isnan(std_glucose(data)) == False
    assert np.round(std_glucose(data)*100)/100 == 86
    ...
    # Tests
    assert np.isnan(std_glucose(data))
