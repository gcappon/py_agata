...
import os
from py_agata.variability import mage_index


def test_mage_index():
    """
    Unit test of mage_index function.
    ...
    #Tests
    assert np.isnan(mage_index(data)) == False
    assert np.round(mage_index(data) * 1000) / 1000 == 160.638
