...
import os
from py_agata.variability import mage_minus_index


def test_mage_minus_index():
    """
    Unit test of mage_minus_index function.
    ...
    # data = pd.read_csv(os.path.join(os.path.abspath(''),'example','data','data.csv'))
    # d = {'t': time_range, 'glucose': data.glucose.values}
    # data = pd.DataFrame(data=d)

    #Tests
    assert np.isnan(mage_minus_index(data)) == False
    assert np.round(mage_minus_index(data) * 1000) / 1000 == 160.768
    ...
    # Tests
    assert np.isnan(mage_minus_index(data))
