...
from py_agata.variability import std_glucose_roc


def test_std_glucose_roc():
    """
    Unit test of std_glucose_roc function.
    ...
    #Tests
    assert np.isnan(std_glucose_roc(data)) == False
    assert np.round(std_glucose_roc(data)*1000)/1000 == 2.332
    ...
    # Tests
    assert np.isnan(std_glucose_roc(data))
