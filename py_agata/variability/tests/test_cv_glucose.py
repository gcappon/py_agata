...
from py_agata.variability import cv_glucose


def test_cv_glucose():
    """
    Unit test of cv_glucose function.
    ...
    #Tests
    assert np.isnan(cv_glucose(data)) == False
    assert np.round(cv_glucose(data)*100)/100 == 62.32
    ...
    # Tests
    assert np.isnan(cv_glucose(data))
