...
import os
from py_agata.variability import sdw_index


def test_sdw_index():
    """
    Unit test of sdw_index function.
    ...
    #Tests
    assert np.isnan(sdw_index(data)) == False
    assert np.round(sdw_index(data) * 1000) / 1000 == 69.972
    ...
    # Tests
    assert np.isnan(sdw_index(data))