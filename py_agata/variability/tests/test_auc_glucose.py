...
from py_agata.variability import auc_glucose


def test_auc_glucose():
    """
    Unit test of auc_glucose function.
    ...
    #Tests
    assert np.isnan(auc_glucose(data)) == False
    assert np.round(auc_glucose(data)*100)/100 == 6900
    ...
    # Tests
    assert np.isnan(auc_glucose(data))
