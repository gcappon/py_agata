...
from py_agata.time_in_ranges import time_in_hypoglycemia


def test_time_in_hypoglycemia():
    """
    Unit test of time_in_hypoglycemia function.
    ...
    #Tests
    assert np.isnan(time_in_hypoglycemia(data,'diabetes')) == False
    assert time_in_hypoglycemia(data, 'diabetes') == 30

    assert np.isnan(time_in_hypoglycemia(data, 'pregnancy')) == False
    assert time_in_hypoglycemia(data, 'pregnancy') == 30
    ...
    # Tests
    assert np.isnan(time_in_hypoglycemia(data))
