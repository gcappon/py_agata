...
from py_agata.time_in_ranges import time_in_l2_hypoglycemia


def test_time_in_l2_hypoglycemia():
    """
    ...
    #Tests
    assert np.isnan(time_in_l2_hypoglycemia(data,'diabetes')) == False
    assert time_in_l2_hypoglycemia(data, 'diabetes') == 10
    ...
    # Tests
    assert np.isnan(time_in_l2_hypoglycemia(data))
